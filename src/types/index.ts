...
// ---- Pipeline (funil de leads) — portado do Fan Mídias, single-tenant ----
...
export interface Lead {
  ...
  owner: string | null
  notes: string | null
  temperature: number          // 0 (frio) a 5 (super quente)
...
// Métricas do pipeline calculadas no servidor (sem tabela pipeline_stages;
// probabilidade fixa por status — ver src/lib/pipeline.ts).
export interface PipelineMetrics {
  open_value: number     // soma do valor estimado dos leads em aberto
  closed_value: number   // soma do valor dos leads fechados
  win_rate: number       // fechados / (fechados + perdidos), 0 a 1
  won: number
  lost: number
  forecast: number       // soma ponderada (valor × probabilidade) dos leads em aberto
}
